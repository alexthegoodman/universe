...
export class BreedingSystem {
  ...
  private readonly MIN_HEALTH_FOR_BREEDING = 15;
  private readonly MIN_ENERGY_FOR_BREEDING = 15;
  private readonly MIN_HAPPINESS_FOR_BREEDING = 15;
  ...
  canBreed(animal: Animal): { canBreed: boolean; reason?: string } {
    ...
    if (lastBreeding && Date.now() - lastBreeding < this.BREEDING_COOLDOWN) {
      const remainingTime =
        this.BREEDING_COOLDOWN - (Date.now() - lastBreeding);
      return {
        canBreed: false,
        reason: `Breeding cooldown: ${Math.ceil(
  ...
  findCompatibleMates(
    ...
    const breedingPairs: BreedingPair[] = potentialMates.map((mate) => {
      const compatibility = DNASystem.calculateCompatibility(
        animal.dna,
        mate.dna
      );
      const estimatedOffspring = this.calculateOffspringProbability(
        animal,
        mate
      );
  ...
  private areRelated(animal1: Animal, animal2: Animal): boolean {
    ...
    if (
      animal1.dna.parentIds?.includes(animal2.dna.id) ||
  ...
  private calculateOffspringProbability(
    parent1: Animal,
    parent2: Animal
  ): number {
    // Base probability
  ...
  autoBreeding(animals: Animal[]): BreedingAttempt[] {
    ...
    const availableForBreeding = animals.filter(
      (animal) => this.canBreed(animal).canBreed
    );
    ...
    for (const animal of interestedInBreeding) {
      ...
      const compatibleMates = this.findCompatibleMates(
        animal,
        interestedInBreeding
  ...
  getBreedingStatistics(animals: Animal[]) {
    ...
    const onCooldown = animals.filter(
      (a) => this.getBreedingCooldown(a.id) > 0
    ).length;
    const tooYoung = animals.filter(
      (a) => a.age < this.MIN_AGE_FOR_BREEDING
    ).length;
    const tooOld = animals.filter(
      (a) => a.age > this.MAX_AGE_FOR_BREEDING
    ).length;
    const unhealthy = animals.filter(
      (a) =>
