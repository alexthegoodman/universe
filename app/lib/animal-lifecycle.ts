...
export class AnimalLifecycle {
  ...
  static generateInitialStats(dna: any): AnimalStats {
    ...
    return {
      health: Math.max(20, Math.min(100, base + (dna.resilience - 50) * 0.3 + (Math.random() - 0.5) * variation)),
  ...
  static degradeStats(animal: Animal): AnimalStats {
    ...
    return {
      health: Math.max(0, animal.stats.health - (degradationRates.health * minutesElapsed * totalMultiplier)),
      hunger: Math.min(100, animal.stats.hunger + (degradationRates.hunger * minutesElapsed * totalMultiplier)),
      energy: Math.max(0, animal.stats.energy - (degradationRates.energy * minutesElapsed * totalMultiplier)),
      happiness: Math.max(0, animal.stats.happiness - (degradationRates.happiness * minutesElapsed * totalMultiplier)),
      thirst: Math.min(100, animal.stats.thirst + (degradationRates.thirst * minutesElapsed * totalMultiplier)),
    };
  }
  
  static checkHealth(animal: Animal): { isHealthy: boolean; criticalStats: string[] } {
    const critical = [];
  ...
  static calculateSurvivalPriority(animal: Animal): number {
    ...
    const healthWeight = animal.stats.health < 30 ? (30 - animal.stats.health) * 2 : 0;
    const hungerWeight = animal.stats.hunger > 70 ? (animal.stats.hunger - 70) * 1.5 : 0;
    const thirstWeight = animal.stats.thirst > 70 ? (animal.stats.thirst - 70) * 2 : 0;
    const energyWeight = animal.stats.energy < 30 ? (30 - animal.stats.energy) * 1 : 0;
  ...
  static generateRandomName(): string {
    const prefixes = [
      'Zara', 'Kiko', 'Luna', 'Max', 'Pip', 'Sage', 'Echo', 'Nova',
      'Rex', 'Mira', 'Zuri', 'Finn', 'Skye', 'Orion', 'Ivy', 'Atlas'
    ];
    
    const suffixes = [
      'paw', 'tail', 'whisker', 'heart', 'wing', 'star', 'moon', 'sun',
      'storm', 'flame', 'wave', 'leaf', 'stone', 'frost', 'ember', 'spark'
    ];
