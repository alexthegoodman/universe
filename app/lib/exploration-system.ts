...
export class ExplorationSystem {
  ...
  determineExplorationGoal(animal: Animal, worldState: any): ExplorationGoal {
    ...
    if (animal.stats.hunger > 70) {
      const knownFood = animalMemories.find(
        (m) =>
    ...
    if (animal.stats.thirst > 70) {
      const knownWater = animalMemories.find(
        (m) => m.discoveryType === "water" && m.reliability > 0.5
      );
    ...
    if (animal.dna.curiosity > 60) {
      ...
      for (const dir of directions) {
        ...
        const minDistanceToExplored =
          exploredPositions.length > 0
            ? Math.min(
                ...exploredPositions.map((p) =>
                  this.calculateDistance(testPos, p)
                )
              )
            : Infinity;
  ...
  addMemory(
    ...
    discovery: Omit<
      ExplorationMemory,
      "id" | "animalId" | "timestamp" | "lastVisited"
    >
  ): void {
    ...
    if (existingMemory) {
      ...
      existingMemory.reliability = Math.min(
        1,
        existingMemory.reliability + 0.1
      );
  ...
  private determineRelationship(
    animal1: Animal,
    animal2: Animal
  ): "friendly" | "neutral" | "rival" {
    // Simple relationship logic - can be enhanced
    const personalityDiff =
      Math.abs(
        animal1.dna.personality.aggressive -
          animal2.dna.personality.aggressive +
          (animal1.dna.personality.nurturing -
            animal2.dna.personality.nurturing)
      ) / 2;
