...
export class BuildingSystem {
  ...
  modifyBuilding(
    ...
    if (action.effects.dimensionChanges) {
      Object.keys(action.effects.dimensionChanges).forEach(key => {
        const change = action.effects.dimensionChanges![key as keyof BuildingDimensions];
        if (change) {
          building.dimensions[key as keyof BuildingDimensions] += change;
          changes.dimensions = changes.dimensions || {};
          changes.dimensions[key] = building.dimensions[key as keyof BuildingDimensions];
        }
      });
    }
    ...
    if (action.effects.statChanges) {
      Object.keys(action.effects.statChanges).forEach(key => {
        ...
        if (change) {
          building.stats[key as keyof BuildingStats] = Math.max(0, 
    ...
    return {
      ...
      duration: 8000 + action.requiredMaterials.stone * 1000 + action.requiredMaterials.wood * 500
    };
  }
  ...
  consumeMaterials(animal: Animal, materials: BuildingMaterial): boolean {
    ...
    for (let i = inventory.length - 1; i >= 0 && stoneNeeded > 0; i--) {
      ...
      if (item.type === "stone") {
        ...
        // Update weight
        animal.inventory.currentWeight -= consumed * 3; // Stone weighs 3 units each
    ...
    for (let i = inventory.length - 1; i >= 0 && woodNeeded > 0; i--) {
      ...
      if (item.type === "wood") {
        ...
        // Update weight  
        animal.inventory.currentWeight -= consumed * 2; // Wood weighs 2 units each
  ...
  getAvailableActions(animal: Animal, buildingId?: string): BuildingAction[] {
    ...
    const canCreate = this.checkMaterials(animal, createAction.requiredMaterials);
        ...
        if (distance <= 5) {
          Object.values(this.buildingActions).forEach(action => {
            if (action.type !== "create_building") {
              const canPerform = this.checkMaterials(animal, action.requiredMaterials);
  ...
  getBuildingBonus(animal: Animal): { comfort: number; happiness: number; safety: number } {
