...
export default function Bandit3D({ bandit, onClick }: Bandit3DProps) {
  ...
  useFrame((state) => {
    ...
    const breathingScale = 1 + Math.sin(state.clock.elapsedTime * breathingRate) * (0.08 + aggressionIntensity * 0.1)
    meshRef.current.scale.setScalar(breathingScale * baseScale)
    
    // Menacing sway/prowl motion
    const prowlMotion = Math.sin(state.clock.elapsedTime * 0.8) * 0.15 * aggressionIntensity
    meshRef.current.position.y = Math.abs(prowlMotion) + 0.2
    
    // Subtle rotation indicating restlessness
    const restlessRotation = Math.sin(state.clock.elapsedTime * 0.5) * 0.3 * aggressionIntensity
    ...
    const eyeGlow = 0.5 + Math.sin(state.clock.elapsedTime * 4) * 0.5 * glowIntensity
    if (eyeLeftRef.current && eyeRightRef.current) {
      const eyeMaterial = eyeLeftRef.current.material as THREE.MeshStandardMaterial
      const eyeMaterial2 = eyeRightRef.current.material as THREE.MeshStandardMaterial
  ...
  const getThreatLevel = () => {
    const canAttack = Date.now() - bandit.lastAttackTime > bandit.attackCooldown
  ...
  return (
    <group 
      ref={groupRef}
      position={[bandit.position.x, 0, bandit.position.z]}
    ...
    >
      <mesh ref={meshRef}>
        ...
        <mesh position={[0.3, 0.2, 0.7]} rotation={[0, 0, Math.PI / 4]} scale={[0.1, 0.6, 0.1]}>
          <boxGeometry args={[1, 1, 1]} />
          <meshStandardMaterial color="#1f2937" />
        </mesh>
        <mesh position={[-0.2, 0.1, 0.7]} rotation={[0, 0, -Math.PI / 6]} scale={[0.08, 0.4, 0.1]}>
          <boxGeometry args={[1, 1, 1]} />
          <meshStandardMaterial color="#1f2937" />
        </mesh>
      ...
      <mesh position={[0, 2.5, 0]}>
        ...
        <meshBasicMaterial 
          color={
            threatLevel === 'enraged' ? '#ff0000' :
            threatLevel === 'aggressive' ? '#dc2626' :
            threatLevel === 'wounded' ? '#7f1d1d' :
            threatLevel === 'dead' ? '#374151' :
            '#991b1b'
          }
        />
      </mesh>
