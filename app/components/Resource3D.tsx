...
export function Resource3D({ resource, onClick }: Resource3DProps) {
  ...
  const getResourceAppearance = () => {
    ...
    return {
      ...
      metalness:
        categoryBase.metalness * (resource.rarity === "legendary" ? 1.2 : 1.0),
    };
  };
  ...
  const getGeometry = () => {
    switch (appearance.shape as "sphere" | "box" | "cylinder") {
      ...
      case "box":
        return (
          <boxGeometry
            args={[appearance.size, appearance.size * 0.6, appearance.size]}
          />
        );
      case "sphere":
      default:
  ...
  return (
    ...
    >
      ...
      {resource.harvestable && resource.quantity > 0 && (
        <mesh
          position={[0, appearance.size + 0.1, 0]}
      ...
      {resource.traits?.ancient && resource.traits.ancient > 85 && (
        <mesh
          position={[0, appearance.size + 0.2, 0]}
          rotation={[0, 0, Math.PI / 4]}
        >
