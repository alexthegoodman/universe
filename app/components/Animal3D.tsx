...
export default function Animal3D({
  ...
  const [currentPosition, setCurrentPosition] = useState(
    new THREE.Vector3(animal.position.x, animal.position.y, animal.position.z)
  );
  ...
  useEffect(() => {
    const newLogicalPosition = new THREE.Vector3(
      animal.position.x,
      animal.position.y,
      animal.position.z
    );
  ...
  useFrame((state) => {
    ...
    if (
      ...
      if (distance > 0.1) {
        ...
        const walkIntensity =
          animal.currentAction === "exploring" ? 0.08 : 0.05;
        meshRef.current.position.y =
          Math.sin(state.clock.elapsedTime * 6) * walkIntensity + 0.3;
      ...
      switch (animal.currentAction) {
        ...
        case "playing":
          ...
          const playBounce =
            Math.abs(Math.sin(state.clock.elapsedTime * 5)) * 0.8;
        ...
        case "socializing":
          ...
          meshRef.current.position.y =
            Math.abs(Math.sin(state.clock.elapsedTime * 2)) * 0.2;
          break;
    ...
    >
      ...
      {animal.currentAction !== "idle" && (
        <mesh position={[0, 2.5, 0]}>
          ...
          <meshBasicMaterial
            color={
              animal.currentAction === "sleeping"
                ? "#8b5cf6"
                : animal.currentAction === "eating"
                ? "#10b981"
                : animal.currentAction === "drinking"
                ? "#3b82f6"
                : animal.currentAction === "playing"
                ? "#f59e0b"
                : animal.currentAction === "exploring"
                ? "#06b6d4"
                : animal.currentAction === "socializing"
                ? "#8b5cf6"
                : animal.currentAction === "working"
                ? "#f97316"
                : animal.currentAction === "mating"
                ? "#ec4899"
                : "#6b7280"
